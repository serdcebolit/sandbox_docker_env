...
class AdminerLoginServers {
	...
	*/
	...
	function getOptions($ar)
	{
		$s = '';

		foreach ($ar as $value)
		{
			$s .= "<option value='${value['server']}'>${value['name']}</option>\n";
		}
	...
	function loginFormField($name, $heading, $value) {
		...
		if ($name == 'driver') {
			return '';
		} elseif ($name == 'server' && count($this->servers)) {
			return $heading . "<select name='auth[server]'>" . $this->getOptions($this->servers) . "</select>\n";
		}
	}
